...
with st.sidebar:
    ...
    min_prob = st.slider("Minimum historical probability", 0.40, 0.90, 0.55, 0.01)
    refresh = st.button("Refresh market data")

cfg = StrategyConfig(target_pct=target, stop_pct=stop, max_hold_days=hold, min_probability=min_prob)
...
if st.button("Run daily scan", type="primary"):
    ...
    if ranked.empty:
        ...
        display = ranked[["symbol", "close", "win_probability", "expected_value", "samples", "rsi", "vol_ratio", "ret_5", "ret_20"]].copy()
        ...
        st.caption("Historical probability is target-before-stop among comparable historical signals. It is not a forecast guarantee.")
else:
...
st.markdown("""
- Run the walk-forward backtest across multiple market regimes.
